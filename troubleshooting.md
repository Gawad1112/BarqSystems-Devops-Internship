...
- Retest evidence: Configuration change verified in the diff (1431fca). Formally retested on 2026-09-11 using the exact procedure this entry specified. Created a record through `POST /records` titled "persistence-across-recreation", which returned id 7. Confirmed present via `GET /records`. Ran `docker compose -p barq-assessment up -d --force-recreate app-01 app-02 postgres`, which destroyed and rebuilt all three containers while leaving the named volume `postgres-data` untouched. `docker compose ps` afterwards showed app-01, app-02 and postgres all created 5 seconds earlier, while nginx and redis still showed their original 34-hour uptime — confirming only the targeted containers were recreated. `GET /records` then returned all seven records including id 7. The record survived container recreation, proving the volume is now mounted at the real PGDATA rather than at `/var/lib/postgresql/backup`. Separately confirmed to survive a full host reboot on 2026-09-10.
